...
# Process hầu hết bind trên IP của chính máy nó đang chạy? [id:2461 order:5]
...
# `0.0.0.0` nghĩa là gì? [id:2462 order:6]
...
# Máy nhận được request từ internet và máy không nhận được thì khác nhau chỗ nào? Gọi là gì? [id:2463 order:7]
...
# Local rule là gì? [id:2464 order:8]
...
# WiFi profile là gì? [id:2466 order:10]
...
# Ý nghĩa của Private/Public trong WiFi profile? [id:2467 order:11]
...
# Các lợi ích cụ thể mà Public/Private profile mang lại? [id:2468 order:12]
...
# Khi request từ A đến B thì TCP connection được thiết lập có phải không? [id:2469 order:13]
...
# Firewall là tập hợp các rule, hay còn gì khác? [id:2470 order:14]
...
# Rule chỉ có 2 loại là allow/block? [id:2471 order:15]
...
# Có 2 loại rule chính là local rule và GPO rule phải k? [id:2472 order:16]
...
# WiFi profile chỉ apply cho 1 WiFi cụ thể thôi phải k? [id:2475 order:19]
...
# IT helpdesk tạo GPO, còn user tạo local rule? [id:2478 order:22]
...
# Chủ WiFi có thể tạo GPO rule không? [id:2479 order:23]
...
<!--# Windows Domain là gì? [id:2480 order:24]
...
# Tại sao Windows Firewall block inbound từ external device? [id:2481 order:25]
...
# Firewall xác định device là external device bằng cách nào? [id:2482 order:26]
...
# Trong WiFi profile Public, local rule luôn bị bỏ qua phải không? [id:2483 order:27]
không,
...
# "Domain" trong networking có mấy nghĩa? [id:2484 order:28]
3 nghĩa phổ biến:
...
# Ai tạo ra local rule? [id:2485 order:29]
...
# Ví dụ về WiFi profile Public/Private? [id:2486 order:30]
...
# Network discovery là gì? [id:2487 order:31]
...
# Khi thiết bị A bị ẩn khỏi network discovery thì các máy cùng mạng không thể giao tiếp được với A? [id:2488 order:32]
...
# Tại sao tắt local rules lại bảo vệ user? [id:2489 order:33]
...
# Tắt local rules không có quá nhiều ý nghĩa trong việc bảo vệ user thông thường? [id:2490 order:34]
...
# Rules thường được dùng khi nào? [id:2491 order:35]
...
# Mặc định khi kết nối WiFi mới thì profile là Public? [id:2492 order:36]
...
# Người dùng bình thường sẽ không cảm nhận được sự khác biệt? [id:2493 order:37]
...
# Tại sao profile Private cho phép local rule còn Public thì không? [id:2494 order:38]
...
# Test từ laptop thành công nhưng phone vẫn không reach được — tại sao? [id:2495 order:39]
...
# Tại sao đổi sang Private profile giải quyết vấn đề API pending? [id:2496 order:40]
...
# Mặc định thì Public profile sẽ tắt local rules? [id:2497 order:41]
...
# Windows Firewall chỉ đơn giản là tập hợp local rule + GPO rule? [id:2498 order:42]
...
# Rule là thành phần chính của policy (GPO)? [id:2499 order:43]
...
# Giải thích "inbound" trong rule "allow TCP port 5000 inbound từ mọi IP"? [id:2500 order:44]
...
# Đổi WiFi từ Public → Private có ý nghĩa gì với dev? [id:2501 order:45]
...
# Thường thì Public profile sẽ tắt local rule? [id:2502 order:46]
