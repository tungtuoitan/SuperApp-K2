...
# Tại sao Frontend không nên dùng id_token để gọi Backend API? [id:846 order:0]
Vì aud (audience) của id_token là Frontend, không phải Backend → Backend sẽ reject.

# thành phần của header? [id:855 order:1]
- chứa loại thuật toán mã hoá và token type

# Signature trong JWT có vai trò gì? [id:844 order:1]
Dùng để xác minh token do ai ký và dữ liệu có bị thay đổi không.

# các thành phần của body? [id:856 order:2]
