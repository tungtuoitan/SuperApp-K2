...
# Audience (aud) trong token là gì? [id:46 order:0]
là Đối tượng nhận token 
(thường là URL của API).

# Tại sao Frontend không nên dùng id_token để gọi Backend API? [id:52 order:0]
Vì aud (audience) của id_token là Frontend, không phải Backend → Backend sẽ reject.

# thành phần của header? [id:28 order:1]
- chứa loại thuật toán mã hoá và token type

# Signature trong JWT có vai trò gì? [id:47 order:1]
Dùng để xác minh token do ai ký và dữ liệu có bị thay đổi không.

# các thành phần của body? [id:29 order:2]
