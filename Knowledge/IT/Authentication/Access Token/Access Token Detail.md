...
# Audience (aud) trong token là gì? [id:1034 order:1]
là Đối tượng nhận token
(thường là URL của API).

# Tại sao Frontend không nên dùng id_token để gọi Backend API? [id:1035 order:2]
Vì aud (audience) của id_token là Frontend, không phải Backend → Backend sẽ reject.

# thành phần của header? [id:1036 order:3]
- chứa loại thuật toán mã hoá và token type

# Signature trong JWT có vai trò gì? [id:1037 order:4]
Dùng để xác minh token do ai ký và dữ liệu có bị thay đổi không.

# các thành phần của body? [id:1038 order:5]
...
# PKCE là gì và tại sao dùng trong SPA? [id:1039 order:6]
Proof Key for Code Exchange – cơ chế bảo mật giúp SPA (Frontend) không cần dùng client_secret.

# id_token và access_token khác nhau như thế nào? [id:1040 order:7]
id_token: client dùng nó để biết được user vừa login là ai
access_token: Dùng để gọi API

# OAuth 2.0 và OIDC là gì? [id:1041 order:8]
là Framework tiêu chuẩn dùng để uỷ quyền.

# payload trong access token chứa gì? [id:1042 order:9]
- chứa thông tin về user/app và quyền truy cập.

# OIDC là gì? [id:1043 order:10]
là 1 layer được xây dựng trên nền tảng OAuth 2.0, dùng để xác thực

# encrypt là gì? [id:1044 order:11]
là mã hóa

# các Claims liên quan đến user/app trong Payload [id:1045 order:12]
sub: ID của user hoặc service principal
oid: Object ID trong Azure
appid: ID của application (client)
tid: Tenant ID

# decrypt là gì? [id:1046 order:13]
là giải mã

# ciphertext là gì? [id:1047 order:14]
là dữ liệu đã mã hóa

# Các phương pháp xác thực phổ biến? [id:1048 order:15]
