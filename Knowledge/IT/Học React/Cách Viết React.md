...
# Tại sao lại nên dùng  configuration object để viết repeat markup? [id:773 order:2]
code dễ đọc, dễ sửa hơn do tuân thủ DRY và data-driven

# 4 nguyên tắc viết component? [id:739 order:3]
- chỉ nên làm 1 việc
- logic nhiều thì viết thành hook
- max 5 props
- tránh nest

# vai trò của error-boundary? [id:774 order:3]
tránh crash app toàn bộ

# Cách dùng react-error-boundary? [id:741 order:5]
- bọc chúng ở component dễ gây bug

# dấu hiệu của 1 component đang làm quá nhiều việc? [id:742 order:6]
- nhiều props

# component nhận nhiều props thì sao? [id:743 order:7]
- dễ bị re-render

# vì sao props.A, props.B ít được sử dụng? [id:775 order:8]
...
# 4 nguyên tắc viết FE? [id:761 order:25]
- public api
- Layered Architecture
- Interceptor
- Shared

# Khi nào thì nên move local state lên trên? [id:763 order:27]
- khi cần share với các component khác
- khi nó bị props drilling

# lợi ích của state ở gần nơi sử dụng ? [id:764 order:28]
- code dễ hiểu
- performance tốt

# props drilling là gì? [id:765 order:29]
...
# Cách abstract UI component đúng cách? [id:767 order:31]
- không abstract ngay
- quan sát pattern lặp lại của chúng
- khi đủ 3 usecase giống nhau thì hãy abstract

# 2 extension nên dùng trong FE? [id:776 order:33]
- ESLint
- Prettier

# Nguyên tắc viết code? [id:777 order:34]
- code nên ở gần nơi sử dụng

# layer-based structure là gì? [id:780 order:34]
là mỗi loại file có 1 folder lớn tương ứng

# Layer Architecture là gì? [id:778 order:35]
là kiến trúc phân tầng: app > feature > shared
flow đi theo chiều từ trên xuống

# Feature-based architecture là gì? [id:781 order:35]
là mỗi feature có 1 folder tương ứng

# public api là gì? [id:779 order:36]
là việc import/export giữa các feature thông qua file index

# tradeoff của caching là gì? [id:769 order:37]
dữ liệu có thể bị cũ

# code bị nest thì sao? [id:768 order:38]
