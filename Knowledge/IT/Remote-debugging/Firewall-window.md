...
# Process hầu hết bind trên IP của chính máy nó đang chạy? [id:560 order:4]
...
# `0.0.0.0` nghĩa là gì? [id:561 order:5]
...
# Máy nhận được request từ internet và máy không nhận được thì khác nhau chỗ nào? Gọi là gì? [id:562 order:6]
...
# Local rule là gì? [id:563 order:7]
...
# WiFi profile là gì? [id:565 order:9]
...
# Ý nghĩa của Private/Public trong WiFi profile? [id:566 order:10]
...
# Các lợi ích cụ thể mà Public/Private profile mang lại? [id:567 order:11]
...
# Khi request từ A đến B thì TCP connection được thiết lập có phải không? [id:568 order:12]
...
# Firewall là tập hợp các rule, hay còn gì khác? [id:569 order:13]
...
# Rule chỉ có 2 loại là allow/block? [id:570 order:14]
...
# Có 2 loại rule chính là local rule và GPO rule phải k? [id:571 order:15]
...
# WiFi profile chỉ apply cho 1 WiFi cụ thể thôi phải k? [id:574 order:18]
...
# IT helpdesk tạo GPO, còn user tạo local rule? [id:577 order:21]
...
# Chủ WiFi có thể tạo GPO rule không? [id:578 order:22]
...
<!--# Windows Domain là gì? [id:579 order:23]
