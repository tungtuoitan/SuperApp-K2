...
# Process hầu hết bind trên IP của chính máy nó đang chạy? [id:1948 order:5]
...
# `0.0.0.0` nghĩa là gì? [id:1949 order:6]
...
# Máy nhận được request từ internet và máy không nhận được thì khác nhau chỗ nào? Gọi là gì? [id:1950 order:7]
...
# Local rule là gì? [id:1951 order:8]
...
# WiFi profile là gì? [id:1953 order:10]
...
# Ý nghĩa của Private/Public trong WiFi profile? [id:1954 order:11]
...
# Các lợi ích cụ thể mà Public/Private profile mang lại? [id:1955 order:12]
...
# Khi request từ A đến B thì TCP connection được thiết lập có phải không? [id:1956 order:13]
...
# Firewall là tập hợp các rule, hay còn gì khác? [id:1957 order:14]
...
# Rule chỉ có 2 loại là allow/block? [id:1958 order:15]
...
# Có 2 loại rule chính là local rule và GPO rule phải k? [id:1959 order:16]
...
# WiFi profile chỉ apply cho 1 WiFi cụ thể thôi phải k? [id:1962 order:19]
...
# IT helpdesk tạo GPO, còn user tạo local rule? [id:1965 order:22]
...
# Chủ WiFi có thể tạo GPO rule không? [id:1966 order:23]
...
<!--# Windows Domain là gì? [id:1967 order:24]
...
# Tại sao Windows Firewall block inbound từ external device? [id:1968 order:25]
...
# Firewall xác định device là external device bằng cách nào? [id:1969 order:26]
...
# Trong WiFi profile Public, local rule luôn bị bỏ qua phải không? [id:1970 order:27]
không,
...
# "Domain" trong networking có mấy nghĩa? [id:1971 order:28]
3 nghĩa phổ biến:
...
# Ai tạo ra local rule? [id:1972 order:29]
...
# Ví dụ về WiFi profile Public/Private? [id:1973 order:30]
...
# Network discovery là gì? [id:1974 order:31]
...
# Khi thiết bị A bị ẩn khỏi network discovery thì các máy cùng mạng không thể giao tiếp được với A? [id:1975 order:32]
...
# Tại sao tắt local rules lại bảo vệ user? [id:1976 order:33]
...
# Tắt local rules không có quá nhiều ý nghĩa trong việc bảo vệ user thông thường? [id:1977 order:34]
...
# Rules thường được dùng khi nào? [id:1978 order:35]
...
# Mặc định khi kết nối WiFi mới thì profile là Public? [id:1979 order:36]
...
# Người dùng bình thường sẽ không cảm nhận được sự khác biệt? [id:1980 order:37]
...
# Tại sao profile Private cho phép local rule còn Public thì không? [id:1981 order:38]
...
# Test từ laptop thành công nhưng phone vẫn không reach được — tại sao? [id:1982 order:39]
...
# Tại sao đổi sang Private profile giải quyết vấn đề API pending? [id:1983 order:40]
...
# Mặc định thì Public profile sẽ tắt local rules? [id:1984 order:41]
...
# Windows Firewall chỉ đơn giản là tập hợp local rule + GPO rule? [id:1985 order:42]
...
# Rule là thành phần chính của policy (GPO)? [id:1986 order:43]
...
# Giải thích "inbound" trong rule "allow TCP port 5000 inbound từ mọi IP"? [id:1987 order:44]
...
# Đổi WiFi từ Public → Private có ý nghĩa gì với dev? [id:1988 order:45]
...
# Thường thì Public profile sẽ tắt local rule? [id:1989 order:46]
