...
# Process hầu hết bind trên IP của chính máy nó đang chạy? [id:903 order:5]
...
# `0.0.0.0` nghĩa là gì? [id:904 order:6]
...
# Máy nhận được request từ internet và máy không nhận được thì khác nhau chỗ nào? Gọi là gì? [id:905 order:7]
...
# Local rule là gì? [id:906 order:8]
...
# WiFi profile là gì? [id:908 order:10]
...
# Ý nghĩa của Private/Public trong WiFi profile? [id:909 order:11]
...
# Các lợi ích cụ thể mà Public/Private profile mang lại? [id:910 order:12]
...
# Khi request từ A đến B thì TCP connection được thiết lập có phải không? [id:911 order:13]
...
# Firewall là tập hợp các rule, hay còn gì khác? [id:912 order:14]
...
# Rule chỉ có 2 loại là allow/block? [id:913 order:15]
...
# Có 2 loại rule chính là local rule và GPO rule phải k? [id:914 order:16]
...
# WiFi profile chỉ apply cho 1 WiFi cụ thể thôi phải k? [id:917 order:19]
...
# IT helpdesk tạo GPO, còn user tạo local rule? [id:920 order:22]
...
# Chủ WiFi có thể tạo GPO rule không? [id:921 order:23]
...
<!--# Windows Domain là gì? [id:922 order:24]
...
# Tại sao Windows Firewall block inbound từ external device? [id:923 order:25]
...
# Firewall xác định device là external device bằng cách nào? [id:924 order:26]
...
# Trong WiFi profile Public, local rule luôn bị bỏ qua phải không? [id:925 order:27]
không,
...
# "Domain" trong networking có mấy nghĩa? [id:926 order:28]
3 nghĩa phổ biến:
...
# Ai tạo ra local rule? [id:927 order:29]
...
# Ví dụ về WiFi profile Public/Private? [id:928 order:30]
...
# Network discovery là gì? [id:929 order:31]
...
# Khi thiết bị A bị ẩn khỏi network discovery thì các máy cùng mạng không thể giao tiếp được với A? [id:930 order:32]
...
# Tại sao tắt local rules lại bảo vệ user? [id:931 order:33]
...
# Tắt local rules không có quá nhiều ý nghĩa trong việc bảo vệ user thông thường? [id:932 order:34]
...
# Rules thường được dùng khi nào? [id:933 order:35]
...
# Mặc định khi kết nối WiFi mới thì profile là Public? [id:934 order:36]
...
# Người dùng bình thường sẽ không cảm nhận được sự khác biệt? [id:935 order:37]
...
# Tại sao profile Private cho phép local rule còn Public thì không? [id:936 order:38]
...
# Test từ laptop thành công nhưng phone vẫn không reach được — tại sao? [id:937 order:39]
...
# Tại sao đổi sang Private profile giải quyết vấn đề API pending? [id:938 order:40]
...
# Mặc định thì Public profile sẽ tắt local rules? [id:939 order:41]
...
# Windows Firewall chỉ đơn giản là tập hợp local rule + GPO rule? [id:940 order:42]
...
# Rule là thành phần chính của policy (GPO)? [id:941 order:43]
...
# Giải thích "inbound" trong rule "allow TCP port 5000 inbound từ mọi IP"? [id:942 order:44]
...
# Đổi WiFi từ Public → Private có ý nghĩa gì với dev? [id:943 order:45]
...
# Thường thì Public profile sẽ tắt local rule? [id:944 order:46]
