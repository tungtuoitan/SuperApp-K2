...
# Process hầu hết bind trên IP của chính máy nó đang chạy? [id:1416 order:5]
...
# `0.0.0.0` nghĩa là gì? [id:1417 order:6]
...
# Máy nhận được request từ internet và máy không nhận được thì khác nhau chỗ nào? Gọi là gì? [id:1418 order:7]
...
# Local rule là gì? [id:1419 order:8]
...
# WiFi profile là gì? [id:1421 order:10]
...
# Ý nghĩa của Private/Public trong WiFi profile? [id:1422 order:11]
...
# Các lợi ích cụ thể mà Public/Private profile mang lại? [id:1423 order:12]
...
# Khi request từ A đến B thì TCP connection được thiết lập có phải không? [id:1424 order:13]
...
# Firewall là tập hợp các rule, hay còn gì khác? [id:1425 order:14]
...
# Rule chỉ có 2 loại là allow/block? [id:1426 order:15]
...
# Có 2 loại rule chính là local rule và GPO rule phải k? [id:1427 order:16]
...
# WiFi profile chỉ apply cho 1 WiFi cụ thể thôi phải k? [id:1430 order:19]
...
# IT helpdesk tạo GPO, còn user tạo local rule? [id:1433 order:22]
...
# Chủ WiFi có thể tạo GPO rule không? [id:1434 order:23]
...
<!--# Windows Domain là gì? [id:1435 order:24]
...
# Tại sao Windows Firewall block inbound từ external device? [id:1436 order:25]
...
# Firewall xác định device là external device bằng cách nào? [id:1437 order:26]
...
# Trong WiFi profile Public, local rule luôn bị bỏ qua phải không? [id:1438 order:27]
không,
...
# "Domain" trong networking có mấy nghĩa? [id:1439 order:28]
3 nghĩa phổ biến:
...
# Ai tạo ra local rule? [id:1440 order:29]
...
# Ví dụ về WiFi profile Public/Private? [id:1441 order:30]
...
# Network discovery là gì? [id:1442 order:31]
...
# Khi thiết bị A bị ẩn khỏi network discovery thì các máy cùng mạng không thể giao tiếp được với A? [id:1443 order:32]
...
# Tại sao tắt local rules lại bảo vệ user? [id:1444 order:33]
...
# Tắt local rules không có quá nhiều ý nghĩa trong việc bảo vệ user thông thường? [id:1445 order:34]
...
# Rules thường được dùng khi nào? [id:1446 order:35]
...
# Mặc định khi kết nối WiFi mới thì profile là Public? [id:1447 order:36]
...
# Người dùng bình thường sẽ không cảm nhận được sự khác biệt? [id:1448 order:37]
...
# Tại sao profile Private cho phép local rule còn Public thì không? [id:1449 order:38]
...
# Test từ laptop thành công nhưng phone vẫn không reach được — tại sao? [id:1450 order:39]
...
# Tại sao đổi sang Private profile giải quyết vấn đề API pending? [id:1451 order:40]
...
# Mặc định thì Public profile sẽ tắt local rules? [id:1452 order:41]
...
# Windows Firewall chỉ đơn giản là tập hợp local rule + GPO rule? [id:1453 order:42]
...
# Rule là thành phần chính của policy (GPO)? [id:1454 order:43]
...
# Giải thích "inbound" trong rule "allow TCP port 5000 inbound từ mọi IP"? [id:1455 order:44]
...
# Đổi WiFi từ Public → Private có ý nghĩa gì với dev? [id:1456 order:45]
...
# Thường thì Public profile sẽ tắt local rule? [id:1457 order:46]
