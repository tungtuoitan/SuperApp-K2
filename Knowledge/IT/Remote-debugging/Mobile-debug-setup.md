...
# Có mấy cách debug mobile với SuperApp? [id:945 order:1]
2 cách: USB + port forwarding, hoặc cùng WiFi + local IP.

<!--# khi remote-debugging bằng cable thì packet đi từ moble chrome > cable > pc chrome > webpack dev server, còn wifi thì packet đi từ mobile chrome > router > pc chrome > webpack dev server phải không? [id:946 order:2] -->

# Công cụ nào dùng để debug mobile? [id:947 order:3]
`chrome://inspect/#devices` trên laptop.

# Daemon là gì? Đọc là gì? [id:948 order:4]
...
# deamon khác gì job nhỉ? [id:949 order:5]

# Port forwarding trong `chrome://inspect` là gì? [id:950 order:6]
...
# `applicationUrl` trong `launchSettings.json` là gì? [id:951 order:7]
...
# `chrome://inspect/#devices` hoạt động thế nào? [id:952 order:8]
...
# Mỗi app thường chỉ có 1 port để nhận request thôi? [id:953 order:9]
...
# 1 port chỉ bind được 1 process, nhưng 1 process bind được nhiều port? [id:954 order:10]
...
# IP và host có nghĩa tương tự nhau không? [id:955 order:11]
...
<!--# Khi dùng port forwarding, phone truy cập FE bằng URL nào? [id:956 order:12]
`http://localhost:3000`. -->

<!--# Tại sao port forwarding không cần cùng mạng? [id:957 order:13]
Vì traffic đi qua USB cable, không qua WiFi hay Ethernet. Không cần IP, không cần firewall rule, không cần CORS. -->

# USB cable có phải network interface không? [id:958 order:14]
...
<!--# Nếu PC kết nối Ethernet, mobile kết nối WiFi thì chrome://inspect không dùng được? [id:959 order:15]
...
# BE cần sửa file nào để nhận connection từ ngoài (WiFi)? [id:960 order:16]
`launchSettings.json` — đổi `applicationUrl` từ `http://localhost:5000` thành `http://0.0.0.0:5000`.

# Khi forward, phải có app nào đó nhận và forward request? [id:961 order:17]
...
<!--# Port forwarding hoạt động thế nào ở tầng thấp? [id:962 order:18]
...
<!--# Tại sao API pending khi dùng WiFi IP nhưng FE vào được bình thường? [id:963 order:19]
...
# Firewall chỉ áp dụng cho wireless, cable thì không bị check? [id:964 order:20]
...
# API pending có nghĩa là gì? [id:965 order:21]
...
# Khi thiết bị A truy cập B:3000, nghĩa là gửi 1 packet? [id:966 order:22]
...
# Chuyện gì xảy ra khi phone gõ `192.168.2.26:3000`? [id:967 order:23]
