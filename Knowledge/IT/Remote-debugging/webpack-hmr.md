...
# Bundler làm gì? [id:2537 order:2]
...
# webpack-dev-server là gì? [id:2539 order:4]
...
# webpack-dev-server có nằm trong webpack không? [id:2540 order:5]
...
# WebSocket là gì? [id:2541 order:6]
...
# Module trong webpack là gì? [id:2542 order:7]
...
# Công việc của webpack-dev-server? [id:2543 order:8]
...
# HMR dùng giao thức gì để nhận thông báo từ server? [id:2544 order:9]
...
<!--# `WDS_SOCKET_HOST` và `WDS_SOCKET_PORT` là gì? [id:2545 order:10]
Biến env cho webpack-dev-server biết HMR WebSocket nên kết nối tới host và port nào. -->

# WebSocket hoạt động thế nào? [id:2546 order:11]
...
# WebSocket nhanh vì connection luôn mở? [id:2547 order:12]
...
# Khi vào `localhost:3000`, browser lấy HTML/CSS/JS thế nào? [id:2548 order:13]
...
# Khi chạy `localhost:3000`, request có IP nguồn là gì? [id:2549 order:14]
...
# Khi code thay đổi, chuyện gì xảy ra? [id:2550 order:15]
...
# craco.config.js liên quan gì đến debug mobile? [id:2551 order:16]
...
# Tại sao cần `host: "0.0.0.0"` trong craco? [id:2552 order:17]
...
# Tại sao HMR lỗi khi dùng USB debug mà không config? [id:2553 order:18]
...
<!--# `auto://0.0.0.0:0/ws` nghĩa là gì? [id:2554 order:19]
...
# Phone gửi request vào webpack-dev-server thế nào? [id:2555 order:20]
