...
# Bundler làm gì? [id:2043 order:2]
...
# webpack-dev-server là gì? [id:2045 order:4]
...
# webpack-dev-server có nằm trong webpack không? [id:2046 order:5]
...
# WebSocket là gì? [id:2047 order:6]
...
# Module trong webpack là gì? [id:2048 order:7]
...
# Công việc của webpack-dev-server? [id:2049 order:8]
...
# HMR dùng giao thức gì để nhận thông báo từ server? [id:2050 order:9]
...
<!--# `WDS_SOCKET_HOST` và `WDS_SOCKET_PORT` là gì? [id:2051 order:10]
Biến env cho webpack-dev-server biết HMR WebSocket nên kết nối tới host và port nào. -->

# WebSocket hoạt động thế nào? [id:2052 order:11]
...
# WebSocket nhanh vì connection luôn mở? [id:2053 order:12]
...
# Khi vào `localhost:3000`, browser lấy HTML/CSS/JS thế nào? [id:2054 order:13]
...
# Khi chạy `localhost:3000`, request có IP nguồn là gì? [id:2055 order:14]
...
# Khi code thay đổi, chuyện gì xảy ra? [id:2056 order:15]
...
# craco.config.js liên quan gì đến debug mobile? [id:2057 order:16]
...
# Tại sao cần `host: "0.0.0.0"` trong craco? [id:2058 order:17]
...
# Tại sao HMR lỗi khi dùng USB debug mà không config? [id:2059 order:18]
...
<!--# `auto://0.0.0.0:0/ws` nghĩa là gì? [id:2060 order:19]
...
# Phone gửi request vào webpack-dev-server thế nào? [id:2061 order:20]
