...
# Bundler làm gì? [id:2024 order:2]
...
# webpack-dev-server là gì? [id:2026 order:4]
...
# webpack-dev-server có nằm trong webpack không? [id:2027 order:5]
...
# WebSocket là gì? [id:2028 order:6]
...
# Module trong webpack là gì? [id:2029 order:7]
...
# Công việc của webpack-dev-server? [id:2030 order:8]
...
# HMR dùng giao thức gì để nhận thông báo từ server? [id:2031 order:9]
...
<!--# `WDS_SOCKET_HOST` và `WDS_SOCKET_PORT` là gì? [id:2032 order:10]
Biến env cho webpack-dev-server biết HMR WebSocket nên kết nối tới host và port nào. -->

# WebSocket hoạt động thế nào? [id:2033 order:11]
...
# WebSocket nhanh vì connection luôn mở? [id:2034 order:12]
...
# Khi vào `localhost:3000`, browser lấy HTML/CSS/JS thế nào? [id:2035 order:13]
...
# Khi chạy `localhost:3000`, request có IP nguồn là gì? [id:2036 order:14]
...
# Khi code thay đổi, chuyện gì xảy ra? [id:2037 order:15]
...
# craco.config.js liên quan gì đến debug mobile? [id:2038 order:16]
...
# Tại sao cần `host: "0.0.0.0"` trong craco? [id:2039 order:17]
...
# Tại sao HMR lỗi khi dùng USB debug mà không config? [id:2040 order:18]
...
<!--# `auto://0.0.0.0:0/ws` nghĩa là gì? [id:2041 order:19]
...
# Phone gửi request vào webpack-dev-server thế nào? [id:2042 order:20]
