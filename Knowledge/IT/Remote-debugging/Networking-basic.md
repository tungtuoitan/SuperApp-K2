...
# Card mạng là gì? Có phải card vật lý không? [id:976 order:9]
...
# Network interface là gì? [id:977 order:10]
...
# `localhost` tương ứng với IP gì? [id:978 order:11]
`127.0.0.1`. Đây là mapping cố định trong file `hosts` của hệ điều hành — không cần DNS.

# `127.0.0.1` có phải là IP không? [id:979 order:12]
...
# Vai trò của gateway? [id:983 order:16]
...
# 1 máy có nhiều IP không? [id:984 order:17]
...
# `127.0.0.1` là loại IP nào? [id:985 order:18]
...
# IP các thiết bị dùng chung WiFi có đặc điểm gì? [id:986 order:19]
...
# Subnet có phải là 1 phần của IP không? [id:989 order:22]
...
# Subnet của WiFi là gì? Có phải là IP của router không? [id:990 order:23]
...
# IP mặc định của router thường là `192.168.1.1` hay `192.168.0.1` — tại sao? [id:991 order:24]
...
# Sơ đồ thành phần trong mạng nhà đơn giản? [id:992 order:25]
```
[Internet / ISP]
       |
...
<!--# PC dùng cáp Ethernet thì subnet có khác WiFi không? [id:993 order:26]
...
# Thiết bị cùng router thì luôn cùng subnet và ping trực tiếp được? [id:994 order:27]
...
# Tại sao không dùng `ipconfig` để lấy IP WiFi? [id:995 order:28]
...
<!--# `192.168.2.1` có phải IP WiFi của laptop không? [id:996 order:29]
