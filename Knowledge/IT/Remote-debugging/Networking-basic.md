...
# Card mạng là gì? Có phải card vật lý không? [id:1489 order:9]
...
# Network interface là gì? [id:1490 order:10]
...
# `localhost` tương ứng với IP gì? [id:1491 order:11]
`127.0.0.1`. Đây là mapping cố định trong file `hosts` của hệ điều hành — không cần DNS.

# `127.0.0.1` có phải là IP không? [id:1492 order:12]
...
# Vai trò của gateway? [id:1496 order:16]
...
# 1 máy có nhiều IP không? [id:1497 order:17]
...
# `127.0.0.1` là loại IP nào? [id:1498 order:18]
...
# IP các thiết bị dùng chung WiFi có đặc điểm gì? [id:1499 order:19]
...
# Subnet có phải là 1 phần của IP không? [id:1502 order:22]
...
# Subnet của WiFi là gì? Có phải là IP của router không? [id:1503 order:23]
...
# IP mặc định của router thường là `192.168.1.1` hay `192.168.0.1` — tại sao? [id:1504 order:24]
...
# Sơ đồ thành phần trong mạng nhà đơn giản? [id:1505 order:25]
```
[Internet / ISP]
       |
...
<!--# PC dùng cáp Ethernet thì subnet có khác WiFi không? [id:1506 order:26]
...
# Thiết bị cùng router thì luôn cùng subnet và ping trực tiếp được? [id:1507 order:27]
...
# Tại sao không dùng `ipconfig` để lấy IP WiFi? [id:1508 order:28]
...
<!--# `192.168.2.1` có phải IP WiFi của laptop không? [id:1509 order:29]
