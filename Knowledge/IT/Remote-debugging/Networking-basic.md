...
# Card mạng là gì? Có phải card vật lý không? [id:2002 order:9]
...
# Network interface là gì? [id:2003 order:10]
...
# `localhost` tương ứng với IP gì? [id:2004 order:11]
`127.0.0.1`. Đây là mapping cố định trong file `hosts` của hệ điều hành — không cần DNS.

# `127.0.0.1` có phải là IP không? [id:2005 order:12]
...
# Vai trò của gateway? [id:2009 order:16]
...
# 1 máy có nhiều IP không? [id:2010 order:17]
...
# `127.0.0.1` là loại IP nào? [id:2011 order:18]
...
# IP các thiết bị dùng chung WiFi có đặc điểm gì? [id:2012 order:19]
...
# Subnet có phải là 1 phần của IP không? [id:2015 order:22]
...
# Subnet của WiFi là gì? Có phải là IP của router không? [id:2016 order:23]
...
# IP mặc định của router thường là `192.168.1.1` hay `192.168.0.1` — tại sao? [id:2017 order:24]
...
# Sơ đồ thành phần trong mạng nhà đơn giản? [id:2018 order:25]
```
[Internet / ISP]
       |
...
<!--# PC dùng cáp Ethernet thì subnet có khác WiFi không? [id:2019 order:26]
...
# Thiết bị cùng router thì luôn cùng subnet và ping trực tiếp được? [id:2020 order:27]
...
# Tại sao không dùng `ipconfig` để lấy IP WiFi? [id:2021 order:28]
...
<!--# `192.168.2.1` có phải IP WiFi của laptop không? [id:2022 order:29]
