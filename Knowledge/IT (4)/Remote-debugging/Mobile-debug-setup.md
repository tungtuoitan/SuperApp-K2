...
# Có mấy cách debug mobile với SuperApp? [id:3016 order:1]
2 cách: USB + port forwarding, hoặc cùng WiFi + local IP.

<!--# khi remote-debugging bằng cable thì packet đi từ moble chrome > cable > pc chrome > webpack dev server, còn wifi thì packet đi từ mobile chrome > router > pc chrome > webpack dev server phải không? [id:3017 order:2] -->

# Công cụ nào dùng để debug mobile? [id:3018 order:3]
`chrome://inspect/#devices` trên laptop.

# Daemon là gì? Đọc là gì? [id:3019 order:4]
...
# deamon khác gì job nhỉ? [id:3020 order:5]

# Port forwarding trong `chrome://inspect` là gì? [id:3021 order:6]
...
# `applicationUrl` trong `launchSettings.json` là gì? [id:3022 order:7]
...
# `chrome://inspect/#devices` hoạt động thế nào? [id:3023 order:8]
...
# Mỗi app thường chỉ có 1 port để nhận request thôi? [id:3024 order:9]
...
# 1 port chỉ bind được 1 process, nhưng 1 process bind được nhiều port? [id:3025 order:10]
...
# IP và host có nghĩa tương tự nhau không? [id:3026 order:11]
...
<!--# Khi dùng port forwarding, phone truy cập FE bằng URL nào? [id:3027 order:12]
`http://localhost:3000`. -->

<!--# Tại sao port forwarding không cần cùng mạng? [id:3028 order:13]
Vì traffic đi qua USB cable, không qua WiFi hay Ethernet. Không cần IP, không cần firewall rule, không cần CORS. -->

# USB cable có phải network interface không? [id:3029 order:14]
...
<!--# Nếu PC kết nối Ethernet, mobile kết nối WiFi thì chrome://inspect không dùng được? [id:3030 order:15]
...
# BE cần sửa file nào để nhận connection từ ngoài (WiFi)? [id:3031 order:16]
`launchSettings.json` — đổi `applicationUrl` từ `http://localhost:5000` thành `http://0.0.0.0:5000`.

# Khi forward, phải có app nào đó nhận và forward request? [id:3032 order:17]
...
<!--# Port forwarding hoạt động thế nào ở tầng thấp? [id:3033 order:18]
...
<!--# Tại sao API pending khi dùng WiFi IP nhưng FE vào được bình thường? [id:3034 order:19]
...
# Firewall chỉ áp dụng cho wireless, cable thì không bị check? [id:3035 order:20]
...
# API pending có nghĩa là gì? [id:3036 order:21]
...
# Khi thiết bị A truy cập B:3000, nghĩa là gửi 1 packet? [id:3037 order:22]
...
# Chuyện gì xảy ra khi phone gõ `192.168.2.26:3000`? [id:3038 order:23]
