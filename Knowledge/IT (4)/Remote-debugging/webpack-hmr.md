...
# Bundler làm gì? [id:3069 order:2]
...
# webpack-dev-server là gì? [id:3071 order:4]
...
# webpack-dev-server có nằm trong webpack không? [id:3072 order:5]
...
# WebSocket là gì? [id:3073 order:6]
...
# Module trong webpack là gì? [id:3074 order:7]
...
# Công việc của webpack-dev-server? [id:3075 order:8]
...
# HMR dùng giao thức gì để nhận thông báo từ server? [id:3076 order:9]
...
<!--# `WDS_SOCKET_HOST` và `WDS_SOCKET_PORT` là gì? [id:3077 order:10]
Biến env cho webpack-dev-server biết HMR WebSocket nên kết nối tới host và port nào. -->

# WebSocket hoạt động thế nào? [id:3078 order:11]
...
# WebSocket nhanh vì connection luôn mở? [id:3079 order:12]
...
# Khi vào `localhost:3000`, browser lấy HTML/CSS/JS thế nào? [id:3080 order:13]
...
# Khi chạy `localhost:3000`, request có IP nguồn là gì? [id:3081 order:14]
...
# Khi code thay đổi, chuyện gì xảy ra? [id:3082 order:15]
...
# craco.config.js liên quan gì đến debug mobile? [id:3083 order:16]
...
# Tại sao cần `host: "0.0.0.0"` trong craco? [id:3084 order:17]
...
# Tại sao HMR lỗi khi dùng USB debug mà không config? [id:3085 order:18]
...
<!--# `auto://0.0.0.0:0/ws` nghĩa là gì? [id:3086 order:19]
...
# Phone gửi request vào webpack-dev-server thế nào? [id:3087 order:20]
