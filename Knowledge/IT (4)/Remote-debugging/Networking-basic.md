...
# Card mạng là gì? Có phải card vật lý không? [id:3047 order:9]
...
# Network interface là gì? [id:3048 order:10]
...
# `localhost` tương ứng với IP gì? [id:3049 order:11]
`127.0.0.1`. Đây là mapping cố định trong file `hosts` của hệ điều hành — không cần DNS.

# `127.0.0.1` có phải là IP không? [id:3050 order:12]
...
# Vai trò của gateway? [id:3054 order:16]
...
# 1 máy có nhiều IP không? [id:3055 order:17]
...
# `127.0.0.1` là loại IP nào? [id:3056 order:18]
...
# IP các thiết bị dùng chung WiFi có đặc điểm gì? [id:3057 order:19]
...
# Subnet có phải là 1 phần của IP không? [id:3060 order:22]
...
# Subnet của WiFi là gì? Có phải là IP của router không? [id:3061 order:23]
...
# IP mặc định của router thường là `192.168.1.1` hay `192.168.0.1` — tại sao? [id:3062 order:24]
...
# Sơ đồ thành phần trong mạng nhà đơn giản? [id:3063 order:25]
```
[Internet / ISP]
       |
...
<!--# PC dùng cáp Ethernet thì subnet có khác WiFi không? [id:3064 order:26]
...
# Thiết bị cùng router thì luôn cùng subnet và ping trực tiếp được? [id:3065 order:27]
...
# Tại sao không dùng `ipconfig` để lấy IP WiFi? [id:3066 order:28]
...
<!--# `192.168.2.1` có phải IP WiFi của laptop không? [id:3067 order:29]
