...
# SAS và JWT có điểm gì giống nhau về cơ chế? [id:3189 order:1]
Cả hai đều dùng HMAC để ký — sửa thông tin là chữ ký sai, bị từ chối.

# SAS và JWT khác nhau ở chỗ nào? [id:3190 order:2]
JWT nằm trong Authorization header. SAS nằm trên URL.

# JWT dùng để làm gì? [id:3191 order:3]
Xác thực người dùng với backend.

# SAS dùng để làm gì? [id:3192 order:4]
Ủy quyền truy cập file trực tiếp với Azure.

# SAS có stateless không? [id:3193 order:5]
Có.

# Tại sao SAS stateless? [id:3194 order:6]
Azure không cần lưu danh sách SAS — chỉ verify chữ ký là đủ.

# JWT bị lộ nguy hiểm hơn hay SAS bị lộ nguy hiểm hơn? [id:3195 order:7]
JWT bị lộ nguy hiểm hơn.

# Tại sao JWT bị lộ nguy hiểm hơn SAS? [id:3196 order:8]
